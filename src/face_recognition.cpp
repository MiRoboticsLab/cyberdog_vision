...
FaceRecognition::FaceRecognition(const std::string &model_det, const std::string &model_lmk, const std::string &model_feat, const std::string &model_emotion, bool open_emotion)
{
...
int FaceRecognition::GetRecognitionResult(const cv::Mat &img, const std::map<std::string, std::vector<float> >& endlib_feats, std::vector<MatchFaceInfo>& faces_info)
{
    XMImage xm_img;
    img_convert(img, xm_img);
